...
MSG_SCRIPT_NOT_ACTIVED = "[SampDealer] {969595}Скрипт выключен. Чтобы включить, введите: /dealer"
...
function main()
	if not isSampfuncsLoaded() or not isSampLoaded() then 
   	...
   	-- Welcome information
   	sampAddChatMessage("[SampDealer] {969595}Скрипт успешно загружен. Информация — {cccccc}/dealerhelp", COLOR_SCRIPTMSG)
...
function sampev.onServerMessage(color, message)
	...
	if string.find(message, " Вы получили (.*) вирт, от (.*)]") then
		local money, nick, id = string.match(message, " Вы получили (%d+) вирт, от (.*)%[(%d+)%]")
		...
		lua_thread.create(function()
        	wait(1200)
        	sampSendChat(("%s[%d] Спасибо большое за чаевые — [%d вирт / Всего: %d вирт]"):format(nick, id, money, players_tip[nick]))
    	end)
	...
	elseif string.find(message, " SMS: mytips. Отправитель: (.*)") then
		local nick, id = string.match(message, " SMS: mytips. Отправитель: (.*)%[(%d+)%]")
		local tips = 0
		local tips_day = 0
		if CheckTable(nick, players_tip, 1) then
		...
		lua_thread.create(function()
        	wait(1200)
        	sampSendChat(("Чаевые игрока %s[%d] — [За сутки: %d $ / Всего: %d $]"):format(nick, tonumber(id), tips_day, tips))
    	end)
	end
end
...
function dealerHelp()
	...
	sampAddChatMessage("» {969595}/dealer — включить/выключить скрипт", COLOR_SCRIPTMSG)
	sampAddChatMessage("» {969595}/gettips — проверить чаевые", COLOR_SCRIPTMSG)
	sampAddChatMessage("» {969595}/toptips — отправить в чат общий топ по чаевым", COLOR_SCRIPTMSG)
	sampAddChatMessage("» {969595}/toptipsday — отправить в чат топ по чаевым за день", COLOR_SCRIPTMSG)
	sampAddChatMessage(("» {969595}/sms %d mytips — выведет в чат информацию по чаевым игрока"):format(id), COLOR_SCRIPTMSG)
	sampAddChatMessage("»»» {969595}Поддержать автора: {cccccc}5536 9138 3677 4212 {969595}(Tinkoff)", COLOR_SCRIPTMSG)
end

function toggleScript()
	if not toggle_script then
		sampAddChatMessage("[SampDealer] {969595}Скрипт активирован. Чтобы выключить, введите: /dealer", COLOR_SCRIPTMSG)
	else
		sampAddChatMessage("[SampDealer] {969595}Скрипт деактивирован", COLOR_SCRIPTMSG)
	end
...
function getTips()
	...
	sampAddChatMessage(("[SampDealer] {969595}Информация о чаевых — [За сутки: %d $ / Всего: %d $]"):format(day_tips, total_tips), COLOR_SCRIPTMSG)
end
...
function SendTopTips(table_, text_)
	if not table_ then
		sampAddChatMessage("[SampDealer] {969595}Информация о чаевых не найдена", COLOR_SCRIPTMSG)
		return
	end
